...
Switching from File Browser keeps your files, your user accounts and everyone's existing passwords — Quantum reads the File Browser database directly and converts it. **The switch is one-way**: File Browser is end of life, so there is no route back to it. Take a StartOS backup before you switch.
...
Cambiar desde File Browser conserva tus archivos, tus cuentas de usuario y las contraseñas existentes: Quantum lee directamente la base de datos de File Browser y la convierte. **El cambio es irreversible**: File Browser ha llegado al final de su vida útil, así que no hay vuelta atrás. Haz una copia de seguridad de StartOS antes de cambiar.
...
Beim Wechsel von File Browser bleiben Ihre Dateien, Ihre Benutzerkonten und alle vorhandenen Passwörter erhalten — Quantum liest die File-Browser-Datenbank direkt und konvertiert sie. **Der Wechsel ist endgültig**: File Browser wird nicht mehr gepflegt, einen Weg zurück gibt es nicht. Erstellen Sie vor dem Wechsel eine StartOS-Sicherung.
...
Przejście z File Browser zachowuje Twoje pliki, konta użytkowników i istniejące hasła — Quantum odczytuje bazę danych File Browser bezpośrednio i konwertuje ją. **Przejście jest nieodwracalne**: File Browser nie jest już rozwijany, więc nie ma drogi powrotnej. Przed przejściem wykonaj kopię zapasową StartOS.
...
Basculer depuis File Browser conserve vos fichiers, vos comptes d'utilisateur et tous les mots de passe existants : Quantum lit directement la base de données de File Browser et la convertit. **La bascule est définitive** : File Browser est en fin de vie, il n'y a donc pas de retour en arrière. Effectuez une sauvegarde StartOS avant de basculer.
